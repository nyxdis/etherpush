...
string? incoming_dialog(string in_filename, ssize_t in_length)
{
	...
	var dialog = new MessageDialog(null, 0, MessageType.QUESTION, ButtonsType.OK_CANCEL, "Accept file transfer?");
	dialog.format_secondary_text("Filename %s\nSize: %lu", in_filename, in_length);
	if (dialog.run() == ResponseType.OK) {
		var filechooser = new FileChooserDialog("Save file", null, FileChooserAction.SAVE, Stock.CANCEL, ResponseType.CANCEL, Stock.SAVE, ResponseType.ACCEPT);
